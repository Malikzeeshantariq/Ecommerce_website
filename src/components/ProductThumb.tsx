...
function ProductThumb({ product }: { product: Product }) {
    ...
    <Link
     ...
     >
        ...
        <p className="mt-2 texzt-sm text-gray-600 line-clamp-2">{product.description?.map((block) => block._type === "block" ?block.children?.map((child) => child.text).join("") : "").join("") || "No description available"}
        </p>
